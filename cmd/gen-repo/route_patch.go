...
func inferHandlerRouteArg(funcType *ast.FuncType) (routeArgKind, string, bool) {
	...
	switch typed := funcType.Params.List[0].Type.(type) {
	case *ast.SelectorExpr:
		...
		switch {
		case pkgIdent.Name == "db" && typed.Sel.Name == "TrxBuilder":
			return routeArgTrxBuilder, "", true
		case pkgIdent.Name == "service" && strings.HasSuffix(typed.Sel.Name, "Usecase"):
			usecaseName := strings.TrimSuffix(typed.Sel.Name, "Usecase")
	...
	case *ast.StarExpr:
		...
		if !ok || !strings.HasSuffix(ident.Name, "Ctrl") {
			return routeArgUnknown, "", false
		}
		base := strings.TrimSuffix(ident.Name, "Ctrl")
		return routeArgUsecase, lowerCamelCase(base) + "Ctrl", true
	default:
		return routeArgUnknown, "", false
	}
}
...
func inferUsecaseTypeName(funcType *ast.FuncType) string {
	...
	case *ast.SelectorExpr:
		...
		if !strings.HasSuffix(typed.Sel.Name, "Usecase") {
			return ""
		}
...
func mergeHTTPRouteFile(existing string, spec httpRouteSpec) (string, bool, error) {
	...
	varChanged, err := ensureRouteUsecaseVar(file, spec)
	if err != nil {
		return "", false, err
	}
...
func ensureRouteUsecaseVar(file *dst.File, spec httpRouteSpec) (bool, error) {
	...
	varDecl := findRouteVarDecl(initRoute.Body)
	valueSpec := &dst.ValueSpec{
		Names: []*dst.Ident{dst.NewIdent(spec.Usecase.FieldName)},
		Values: []dst.Expr{
			&dst.CallExpr{
				Fun: &dst.SelectorExpr{
					X:   dst.NewIdent(spec.ModuleName),
					Sel: dst.NewIdent(routeUsecaseConstructorName(spec.Usecase)),
				},
				Args: []dst.Expr{
					&dst.SelectorExpr{
						X:   dst.NewIdent("ox"),
						Sel: dst.NewIdent("driver"),
					},
				},
			},
		},
	...
	if varDecl == nil {
		initRoute.Body.List = append([]dst.Stmt{
			&dst.DeclStmt{
				Decl: &dst.GenDecl{
					Tok:    token.VAR,
					Lparen: true,
					Rparen: true,
					Specs:  []dst.Spec{valueSpec},
				},
			},
		}, initRoute.Body.List...)
		return true, nil
	...
	for _, existing := range valueSpecs(varDecl) {
		for _, name := range existing.Names {
			if name != nil && name.Name == spec.Usecase.FieldName {
				return false, nil
			}
		}
	}

	varDecl.Specs = append(varDecl.Specs, valueSpec)
...
func ensureRouteRegistrations(file *dst.File, spec httpRouteSpec) (bool, error) {
	...
	controllerPath := "/" + httpPathName(spec.ModuleName)
	block := findModuleRouteBlock(initRoute.Body, controllerPath)
	if block == nil {
	...
	changed := ensureBlockControllerPath(block, controllerPath)
...
func findRouteVarDecl(body *dst.BlockStmt) *dst.GenDecl {
	if body == nil {
		return nil
	}
...
func routeUsecaseConstructorName(usecase httpUsecaseSpec) string {
	base := strings.TrimSuffix(usecase.TypeName, "Usecase")
...
func findModuleRouteBlock(body *dst.BlockStmt, controllerPath string) *dst.BlockStmt {
	...
	for _, stmt := range body.List {
		...
		if normalizeControllerPath(blockControllerPath(block)) == normalizeControllerPath(controllerPath) {
			return block
		}
	}
	return nil
...
func normalizeControllerPath(path string) string {
	if path == "" {
		return ""
	}
	return "/" + httpPathName(strings.TrimPrefix(strings.TrimSpace(path), "/"))
}

func ensureBlockControllerPath(block *dst.BlockStmt, controllerPath string) bool {
	if block == nil {
		return false
	}
	desired := normalizeControllerPath(controllerPath)
	for _, stmt := range block.List {
		assignStmt, ok := stmt.(*dst.AssignStmt)
		if !ok || len(assignStmt.Rhs) != 1 {
			continue
		...
		if normalizeControllerPath(value) == desired && value != desired {
			literal.Value = strconv.Quote(desired)
			return true
		}
		return false
	}
...
func newModuleRouteBlock(spec httpRouteSpec) *dst.BlockStmt {
	stmts := []dst.Stmt{
		&dst.AssignStmt{
			...
			Tok: token.DEFINE,
			Rhs: []dst.Expr{
				&dst.BasicLit{Kind: token.STRING, Value: strconv.Quote("/" + httpPathName(spec.ModuleName))},
			},
		},
		&dst.AssignStmt{
...
func newRouteStatement(moduleName string, route httpRoute) dst.Stmt {
	...
	case routeArgUsecase:
		handlerArgs = append(handlerArgs, dst.NewIdent(route.UsecaseField))
	}
