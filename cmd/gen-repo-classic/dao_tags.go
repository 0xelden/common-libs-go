...
// loadDTOFieldTags loads struct tags from an existing dto file.
func loadDTOFieldTags(outDir string, table *Table) (map[string]map[string]string, error) {
	...
	path := filepath.Join(outDir, "modules", moduleName, "dto", fmt.Sprintf("%s_dto.go", moduleName))
	data, err := os.ReadFile(path)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil
		}
		return nil, errors.Errorf("read existing dto file: %v", err)
	}

	fset := token.NewFileSet()
	file, err := decorator.ParseFile(fset, path, data, parser.ParseComments)
	if err != nil {
		return nil, errors.Errorf("parse existing dto file: %v", err)
	}

	structName := structNameForTable(table.Name) + "Dto"
	return findStructFieldTags(file, structName), nil
}
...
func mergeDTOStructTags(column string, existing map[string]string) string {
	...
	for _, key := range []string{"form", "json", "db", "query", "validate"} {
		value, ok := tags[key]
		if !ok {
			continue
		}
