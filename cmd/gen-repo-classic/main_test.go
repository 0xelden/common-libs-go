...
func TestGenerateFiles(t *testing.T) {
	...
	if len(files) != 4 {
		t.Fatalf("expected 4 generated files, got %d", len(files))
	}
	fileMap := map[string]GeneratedFile{}
	for _, file := range files {
		fileMap[file.Path] = file
	}
	expectedDTOPath := filepath.Join("modules", "demo_table", "dto", "demo_table_dto.go")
	...
	for _, path := range []string{expectedDTOPath, expectedModelPath, expectedQueryPath, expectedRepoPath} {
		if _, ok := fileMap[path]; !ok {
			t.Fatalf("expected generated file at %s", path)
		}
	}
	dtoFile := fileMap[expectedDTOPath].Content
	if !strings.Contains(dtoFile, "type DemoTableDto struct") {
		t.Fatalf("dto file missing dto struct definition:\n%s", dtoFile)
	}
	if !strings.Contains(dtoFile, "*time.Time") {
		t.Fatalf("dto file should use time for timestamp columns:\n%s", dtoFile)
	}
	if strings.Contains(dtoFile, "Year") {
		t.Fatalf("dto file should exclude generated columns:\n%s", dtoFile)
	}
	if strings.Contains(dtoFile, "uuid.UUID") {
		t.Fatalf("dto file should use string for uuid columns:\n%s", dtoFile)
	}
	...
	if !strings.Contains(modelFile, "dto.DemoTableDto") {
	...
	if !strings.Contains(repoFile, fmt.Sprintf("\"%s/modules/demo_table/dto\"", modulePath)) {
		t.Fatalf("repository file missing module import path:\n%s", repoFile)
	}
	if !strings.Contains(repoFile, "NewDemoTableRepo(driver db.Driver)") {
		t.Fatalf("repository file should accept db.Driver:\n%s", repoFile)
	}
	if !strings.Contains(repoFile, "csvc.Repository[dto.DemoTableDto, model.DemoTable]") {
...
func TestGenerateFilesWithController(t *testing.T) {
	...
	if !strings.Contains(ctrlFile.Content, "service.DemoTableUsecase") {
		t.Fatalf("controller file missing service usecase interface:\n%s", ctrlFile.Content)
	}
	if !strings.Contains(ctrlFile.Content, "editable := dto.DemoTableDto{") {
...
func TestGenerateFilesUseCurrentModuleNaming(t *testing.T) {
	...
	dtoPath := filepath.Join("modules", "menu", "dto", "menu_dto.go")
	if _, ok := paths[dtoPath]; !ok {
		t.Fatalf("expected generated file at %s", dtoPath)
	}
	if !strings.Contains(paths[dtoPath].Content, "type MenuDto struct") {
		t.Fatalf("expected MenuDto struct:\n%s", paths[dtoPath].Content)
	}
...
func TestLoadDTOFieldTags(t *testing.T) {
	tmpDir := t.TempDir()
	dtoDir := filepath.Join(tmpDir, "modules", "demo_table", "dto")
	if err := os.MkdirAll(dtoDir, 0o755); err != nil {
		t.Fatalf("mkdir dto dir: %v", err)
	}
	...
	dtoPath := filepath.Join(dtoDir, "demo_table_dto.go")
	if err := os.WriteFile(dtoPath, []byte(content), 0o644); err != nil {
		t.Fatalf("write dto file: %v", err)
	}
	...
	dtoFile, err := buildDTOFile(table, tags)
	if err != nil {
		t.Fatalf("buildDTOFile returned error: %v", err)
	}
	if !strings.Contains(dtoFile, "validate:\"omitempty,uuid\"") {
		t.Fatalf("dto file missing validate tag:\n%s", dtoFile)
	}
	if !strings.Contains(dtoFile, "json:\"name,omitempty\"") {
		t.Fatalf("dto file missing preserved json tag:\n%s", dtoFile)
	}
}
...
func TestUpdateRegistryFiles(t *testing.T) {
	...
	repositoryFile := `package service
...
type ExistingRepo interface{}
`
	repositoryPath := filepath.Join(serviceDir, "repository.go")
	if err := os.WriteFile(repositoryPath, []byte(repositoryFile), 0o644); err != nil {
		t.Fatalf("write service/repository.go: %v", err)
	}

	usecaseFile := `package service
...
type ExistingUsecase interface {
	Ping(ctx context.Context, param api.IndexParam) (serr serror.SError)
}
`
	usecasePath := filepath.Join(serviceDir, "usecase.go")
	if err := os.WriteFile(usecasePath, []byte(usecaseFile), 0o644); err != nil {
		t.Fatalf("write service/usecase.go: %v", err)
	}
	...
	if len(files) != 2 {
		t.Fatalf("expected 2 registry files, got %d", len(files))
	}

	for _, file := range files {
		targetPath := filepath.Join(tmpDir, file.Path)
		content, shouldWrite, err := renderGeneratedFile(config{}, targetPath, file, file.Content)
		if err != nil {
			t.Fatalf("renderGeneratedFile returned error for %s: %v", file.Path, err)
		}
		if !shouldWrite {
			t.Fatalf("expected renderGeneratedFile to request write for %s", file.Path)
		}

		if file.Path == filepath.Join("service", "repository.go") {
			if !strings.Contains(content, `menudto "gitlab.com/example/project/modules/menu/dto"`) {
				t.Fatalf("repository registry missing dto import:\n%s", content)
			}
			if !strings.Contains(content, "type MenuRepo interface") {
				t.Fatalf("repository registry missing MenuRepo interface:\n%s", content)
			}
		}

		if file.Path == filepath.Join("service", "usecase.go") {
			if !strings.Contains(content, `menumodel "gitlab.com/example/project/modules/menu/model"`) {
				t.Fatalf("usecase registry missing model import:\n%s", content)
			}
			if !strings.Contains(content, "type MenuUsecase interface") {
				t.Fatalf("usecase registry missing MenuUsecase interface:\n%s", content)
			}
			if !strings.Contains(content, "CreateMenu(ctx context.Context, form menudto.MenuDto)") {
				t.Fatalf("usecase registry missing CreateMenu signature:\n%s", content)
			}
		}
	}
}
...
func IndexDemoTable(ctrl service.DemoTableUsecase) gin.HandlerFunc { return nil }
func SaveDemoTable(txb db.TrxBuilder) gin.HandlerFunc { return nil }
func ViewDemoTable(ctrl service.DemoTableUsecase) gin.HandlerFunc { return nil }
...
`
	...
	if !strings.Contains(content, `controller := "/demo_table"`) {
		t.Fatalf("expected module controller block:\n%s", content)
	}
	if !strings.Contains(content, `router.GET("/index", demo_table.IndexDemoTable(ox.demoTableUsecase))`) {
...
func (ox gatewayHandler) initRoute() {
	{
		...
		router.GET("/index", demo_table.IndexDemoTable(ox.demoTableUsecase))
...
`
	...
	if strings.Count(content, `router.GET("/index", demo_table.IndexDemoTable(ox.demoTableUsecase))`) != 1 {
	...
	if !strings.Contains(content, `router.GET("/view/:id", demo_table.ViewDemoTable(ox.demoTableUsecase))`) {
...
func TestPatchServiceHTTPHandlerAddsUsecaseFieldParamAndAssignment(t *testing.T) {
	tmpDir := t.TempDir()
	handlerDir := filepath.Join(tmpDir, "service", "http")
...
import (
	...
	"gitlab.com/example/project/service"
	"github.com/0xelden/common-libs-go/serror"
)

type gatewayHandler struct {
	service       *gin.Engine
	trxBuilder    db.TrxBuilder
	officeUsecase service.OfficeUsecase
}
...
`
	if err := os.WriteFile(filepath.Join(handlerDir, "handler.go"), []byte(existing), 0o644); err != nil {
		t.Fatalf("write handler.go: %v", err)
	}

	generatedHTTP := `package sim_recplan

func IndexSimRecplan(ctrl service.SimRecplanUsecase) gin.HandlerFunc { return nil }
func SaveSimRecplan(txb db.TrxBuilder) gin.HandlerFunc { return nil }
func ViewSimRecplan(ctrl service.SimRecplanUsecase) gin.HandlerFunc { return nil }
`

	content, shouldWrite, err := patchServiceHTTPHandler(tmpDir, generatedHTTP)
	if err != nil {
		t.Fatalf("patchServiceHTTPHandler returned error: %v", err)
	}
	if !shouldWrite {
		t.Fatalf("expected handler patch to request write")
	}

	if !strings.Contains(content, `simRecplanUsecase service.SimRecplanUsecase`) {
		t.Fatalf("expected gatewayHandler field:\n%s", content)
	}
	if !strings.Contains(content, "officeUsecase service.OfficeUsecase,\n\tsimRecplanUsecase service.SimRecplanUsecase,\n") {
		t.Fatalf("expected constructor parameter:\n%s", content)
	}
	if !strings.Contains(content, "officeUsecase:     officeUsecase,\n\t\tsimRecplanUsecase: simRecplanUsecase,\n\t}") {
		t.Fatalf("expected constructor assignment:\n%s", content)
	}
}

func TestPatchServiceHTTPHandlerSkipsExistingUsecaseWiring(t *testing.T) {
	tmpDir := t.TempDir()
	handlerDir := filepath.Join(tmpDir, "service", "http")
	if err := os.MkdirAll(handlerDir, 0o755); err != nil {
		t.Fatalf("mkdir handler dir: %v", err)
...
import "gitlab.com/example/project/service"
...
`
	...
	if shouldWrite {
		t.Fatalf("expected existing handler wiring to be left unchanged:\n%s", content)
	}
}

func TestPatchHTTPServicesConfigAddsImportVarAndGatewayArg(t *testing.T) {
	tmpDir := t.TempDir()
	configDir := filepath.Join(tmpDir, "config", "http")
...
import (
	"github.com/0xelden/common-libs-go/db"
	"gitlab.com/example/project/modules/office"
	servicehttp "gitlab.com/example/project/service/http"
)

func (cfg *Config) RegisterService() error {
	pgDriver, err := db.NewSqlDriverMulti(cfg.MultiPG)
	if err != nil {
		return err
	}

	var (
		officeCtrl = office.NewOfficeUsecase(pgDriver)
	)

	err = servicehttp.NewGatewayHandler(
		cfg.Gin,
		officeCtrl,
	)
	if err != nil {
...
`
	if err := os.WriteFile(filepath.Join(configDir, "services.go"), []byte(existing), 0o644); err != nil {
...
`
	...
	content, shouldWrite, err := patchHTTPServicesConfig(tmpDir, "gitlab.com/example/project", generatedHTTP)
	if err != nil {
		t.Fatalf("patchHTTPServicesConfig returned error: %v", err)
	}
	if !shouldWrite {
		t.Fatalf("expected services config patch to request write")
	}

	if !strings.Contains(content, `"gitlab.com/example/project/modules/sim_recplan"`) {
		t.Fatalf("expected module import:\n%s", content)
	}
	if !strings.Contains(content, `simRecplanCtrl = sim_recplan.NewSimRecplanUsecase(pgDriver)`) {
		t.Fatalf("expected usecase constructor in var block:\n%s", content)
	}
	if !strings.Contains(content, "officeCtrl,\n\t\tsimRecplanCtrl,\n\t)") {
		t.Fatalf("expected NewGatewayHandler call arg:\n%s", content)
	}
}
...
func TestBuildPostmanCollectionCreatesStandaloneCollectionAndPreservesMetadata(t *testing.T) {
	...
	if index.Request == nil || index.Request.URL.Raw != "{{gateway}}/demo_table/index?page=1&size=10" {
				...
				Item: []postmanItem{
					{
						...
						Request: &postmanRequest{
							Method: "POST",
							URL: postmanURL{
								Raw:  "{{gateway}}/demo_table/add",
								Host: []string{"{{gateway}}"},
								Path: []string{"demo_table", "add"},
							},
						},
					},
					{
						...
						Request: &postmanRequest{
							Method: "POST",
							URL: postmanURL{
								Raw:  "{{gateway}}/demo_table/custom",
								Host: []string{"{{gateway}}"},
								Path: []string{"demo_table", "custom"},
							},
						},
					},
				},
...
func TestBuildPostmanCollectionPrependsURLPathWhenConfigured(t *testing.T) {
	...
	if index.Request.URL.Raw != "{{gateway}}/erp/hkhd/demo_table/index?page=1&size=10" {
		t.Fatalf("unexpected index request url: %#v", index.Request.URL)
	}
	if strings.Join(index.Request.URL.Path, "/") != "erp/hkhd/demo_table/index" {
